...
def plot_fp(boundary, boxes, types, doors=[], windows=[], wall_thickness=6, fontsize=0, keep_box=False, alpha=1.0, ax=None):
    ...
    for k in range(len(boxes)):
        ...
        if poly[k].area == 0: 
            print(f'plotting empty box {k}!')
            continue

        if keep_box:
            poly[k] = geometry.box(*poly[k].bounds)
            x, y = poly[k].exterior.xy
            ax.fill(x, y, fc=cmap[types[k]], ec=cmap[16], alpha=alpha,
                    lw=wall_thickness, joinstyle='round')
        else:
            ...
            if poly[k].geom_type == 'Polygon':
                ...
                ax.fill(x, y, fc=cmap[types[k]], ec=cmap[16], alpha=alpha,
                        lw=wall_thickness, joinstyle='round')
            elif poly[k].geom_type == 'MultiPolygon' or poly[k].geom_type == 'GeometryCollection':
                for p in poly[k].geoms:
                ...
                print(f"Unhandled geometry type at index {k}: {poly[k].geom_type}")
